...
@router.callback_query(F.data == x_channels_change_rating_winrate_cb)
async def rating_x_channels_menu(cb: types.CallbackQuery, state: FSMContext):
    await state.set_state(None)
    await cb.message.edit_text(
        "<b>🏆 Изменение рейтинга/винрейта X каналов</b>\n\n"
        "Выберите действие:",
        reply_markup=Markup.rating_winrate_x_channels_menu()
    )


@router.callback_query(F.data == x_channels_rating_winrate_cb)
async def show_zero_rating_x_channels(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=await Markup.channel_list_for_rating(channels, x_channels_change_rating_winrate_cb, True)
    )
...
async def show_all_x_channels_for_reevaluation(cb: types.CallbackQuery, state: FSMContext):
    ...
    channels = await XChannelRepo.get_by_rating_greater_than(-1)
    ...
    await cb.message.edit_text(
        ...
        reply_markup=await Markup.channel_list_for_rating(channels, x_channels_change_rating_winrate_cb)
    )
...
async def choose_rating_for_x_channel(cb: types.CallbackQuery, state: FSMContext):
    await state.set_state(None)
    f_data = cb.data.replace("rate_x_channel_", "")
    if 'winrate_' in f_data:
        channel_id = int(f_data.replace("winrate_", ""))
        await state.set_state(XChannelStates.add_raiting_winrate)
    else:
        channel_id = int(f_data)
...
async def handle_x_channel_rating_winrate_selection(cb: types.CallbackQuery, callback_data: XChannelRatingCb, state: FSMContext):
    ...
    if success:
        ...
        await cb.message.edit_text(
            ...
            "Введите винрейт для этого канала:",
