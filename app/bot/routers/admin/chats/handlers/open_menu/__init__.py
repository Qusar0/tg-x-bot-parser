...
async def central_chats_menu(cb: types.CallbackQuery, state: FSMContext):
    ...
    if central_chats:
        ...
        for num, chat in enumerate(central_chats, start=1):
            user = await UserRepo.get_by_telegram_id(chat.telegram_id)
            if user:
                template += f"\n<i>{num}.</i> <a href='{user.exists_link_entity}'>{user.full_name}</a> | <code>{chat.telegram_id}</code>"  # noqa
            else:
                template += f"\n<i>{num}.</i> <a href='{chat.entity}'>{chat.title}</a> | <code>{chat.telegram_id}</code>"
