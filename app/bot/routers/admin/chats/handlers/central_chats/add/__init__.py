...
async def add_me_handler(cb: types.CallbackQuery, state: FSMContext, user: User):
    ...
    await ChatRepo.add_central_chat(user.telegram_id, user.full_name, user.exists_link_entity)
...
async def add_central_handler(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=await Markup.back_central_chat(is_add=True),
    )
...
async def add_central_scene(message: types.Message):
    ...
    if not chat_entities:
        await message.answer("⚠️ Пожалуйста, укажите корректный чат", reply_markup=await Markup.back_central_chat(is_add=True))
        return
    ...
    try:
        ...
        if not chat:
            await message.answer("⚠️ Чат не найден", reply_markup=await Markup.back_central_chat(is_add=True))
            return

        if not isinstance(chat, pyrogram_types.ChatPreview):
            existing = await ChatRepo.get_by_telegram_id(chat.id)
            if existing and existing.is_central:
        ...
        if isinstance(chat, pyrogram_types.User):
            if chat.is_bot:
                await message.answer(
                    "⚠️ Нельзя указывать других ботов", reply_markup=await Markup.back_central_chat(is_add=True)
                )
                return
        ...
        await ChatRepo.add_central_chat(chat.id, chat.title, chat_entity)
        ...
        logger.warning(ex)
        await message.answer(f"⚠️ Что-то пошло не так {ex}", reply_markup=await Markup.back_central_chat(is_add=True))
